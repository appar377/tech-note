...
description: Ubuntu系Linux上でMetaTrader 5を動かし、Wine、GUIセッション、Python API、systemdをどう接続するかをシリーズ全体の地図として整理する。
date: 2026-06-28
tags:
  - Ubuntu
  - Linux
...
- Wine、display layer、Python API、systemdの境界
- シリーズをどの順番で読めばよいか
...
| 03 | headless設計 | Ubuntu ServerでDISPLAYをどう用意するか |
| 04 | Wine + MT5 | prefix、terminal path、初回ログイン |
| 05 | Python API | MetaTrader5 package、initialize、bridge構成 |
| 06 | systemd | display、MT5、botの起動順序とログ |
| 07 | まとめ | 最終構成、メリット、デメリット、失敗集 |
...
手動起動では動いても、systemdでは動かないことがあります。その場合、Pythonコードの問題に見えて、実際には`DISPLAY`、`WINEPREFIX`、`HOME`、`PATH`の差分だった、という切り分けが必要になります。
...
flowchart TD
  ...
  B --> C["Wine"]
  C --> D["MetaTrader 5 terminal"]
  D --> E["Python API / worker"]
  E --> F["bot process"]
  A --> G["systemd"]
  G --> B
  G --> C
  G --> F
  G --> H["journald / application logs"]
```
...
- 本番構成ではUbuntu Serverに必要最小限のdisplay layerを足し、MT5とbotをsystemdで管理する設計が現実的。
