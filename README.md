...
`series` はタグやカテゴリの別名ではなく、1つのテーマを章立てで読み進める「技術書ページ」として扱います。

- 記事側のFrontMatterでは、どのシリーズの何章目かだけを指定します。
- シリーズ自体の目的、対象読者、前提知識、到達点、章構成、補足解説、参考リンクは `lib/series-books.ts` に定義します。
- シリーズページには公開済み記事だけを表示します。未公開予定章やComing soonは出しません。
- `series.order` は章の並び順に使います。表示上の章番号は公開済み記事の並びから採番します。
